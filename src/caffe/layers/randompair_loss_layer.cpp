...
void RandomPairLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void RandomPairLossLayer<Dtype>::Forward_cpu(
  ...
  for(int i=0; i<pos_num; i++)
  {
     caffe_sub(
        channels, 
        bottom[0]->cpu_data() + pairdist_pos_[i].first*channels, 
        bottom[0]->cpu_data() + pairdist_pos_[i].second*channels, 
        diff_.mutable_cpu_data());
     pairdist_pos_[i].dist = caffe_cpu_dot(channels, diff_.cpu_data(), diff_.cpu_data());
  }
  for(int i=0; i<neg_num; i++)
  {
     caffe_sub(
        channels, 
        bottom[0]->cpu_data() + pairdist_neg_[i].first*channels, 
        bottom[0]->cpu_data() + pairdist_neg_[i].second*channels, 
        diff_.mutable_cpu_data());
     pairdist_neg_[i].dist = caffe_cpu_dot(channels, diff_.cpu_data(), diff_.cpu_data());
  }
...
template <typename Dtype>
void RandomPairLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(RandomPairLossLayer);
#endif

INSTANTIATE_CLASS(RandomPairLossLayer);
REGISTER_LAYER_CLASS(RandomPairLoss);
