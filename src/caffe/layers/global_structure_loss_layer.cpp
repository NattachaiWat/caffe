...
void GlobalStructureLossLayer<Dtype>::Forward_cpu( 
    ...
    for (int i=0; i<C; i++)
    {
        ...
        caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, D, C, 1,
...
void GlobalStructureLossLayer<Dtype>::Backward_cpu( const vector<Blob<Dtype>*>& top,
    ...
    Dtype* data2 = diff_xi_center_.mutable_cpu_data(); // the intra diff
    ...
    Dtype* data4 = delta_flag_.mutable_cpu_data();
    Dtype* data5 = diff_centers_centers_.mutable_cpu_data();
    // set the update
    for(int i=0; i<num; i++)
    {
