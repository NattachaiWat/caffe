...
#include "caffe/layers/multilabel_loss_layer.hpp"
...
//  Dtype weight[] = {0.4865, 0.0789, 0.6699, 0.1386, 0.1123, 0.2427, 0.7945, 0.1314, 0.1352, 0.1820, 0.0539};
  ...
  bool primal = this->layer_param_.multilabel_loss_param().wtype();
  Dtype* weight = NULL;
  if ( weight_size > 0 )
  {
    ...
    for(int i=0; i<count; i++)
    {
        if(target[i] != 0)
        {
            if( target[i] > 0)
            {
                if(primal)
                {
                    bottom_diff[i] = (sigmoid_output_data[i] - 1)*(1 - weight[i%dim]);
            ...
            else
            {
                if(primal)
                {
                    bottom_diff[i] = sigmoid_output_data[i] * (weight[i%dim]);
