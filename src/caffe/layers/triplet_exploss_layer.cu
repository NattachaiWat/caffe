...
namespace caffe {
  ...
  void TripletExpLossLayer<Dtype>::Forward_gpu(
    ...
    for (int i = 0; i < bottom[0]->num(); ++i) {
      mdist = std::max(margin + dist_sq_ap_.cpu_data()[i] - dist_sq_an_.cpu_data()[i], Dtype(0.0));
      mdist = mdist/2;
  ...
  template <typename Dtype>
  void TripletExpLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  INSTANTIATE_LAYER_GPU_FUNCS(TripletExpLossLayer);
