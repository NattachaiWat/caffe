...
class MultiImageDataLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ...
  virtual void SetUp() {
    ...
    //
    cout << "lidangwei: " << filename_ << endl;
    std::ofstream reshapefile(filename_reshape_.c_str(), std::ofstream::out);
    LOG(INFO) << "Using temporary file " << filename_reshape_;
    reshapefile << EXAMPLES_SOURCE_DIR "images/cat.jpg " << 0 << endl;
    reshapefile << EXAMPLES_SOURCE_DIR "images/fish-bike.jpg " << 1 << endl;
    reshapefile.close();
  }
...
TYPED_TEST(MultiImageDataLayerTest, TestRead) {
  ...
  MultiImageDataParameter* multiimage_data_param = param.mutable_multiimage_data_param();
  multiimage_data_param->set_batch_size(5);
  multiimage_data_param->set_source(this->filename_.c_str());
  multiimage_data_param->set_shuffle(false);
...
TYPED_TEST(MultiImageDataLayerTest, TestResize) {
  ...
  multiimage_data_param->set_batch_size(5);
  multiimage_data_param->set_source(this->filename_.c_str());
  multiimage_data_param->set_new_height(256);
  multiimage_data_param->set_new_width(256);
  multiimage_data_param->set_shuffle(false);
  MultiImageDataLayer<Dtype> layer(param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_data_->num(), 5);
...
TYPED_TEST(MultiImageDataLayerTest, TestReshape) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter param;
  MultiImageDataParameter* multiimage_data_param = param.mutable_multiimage_data_param();
  multiimage_data_param->set_batch_size(1);
  multiimage_data_param->set_source(this->filename_reshape_.c_str());
  multiimage_data_param->set_shuffle(false);
  MultiImageDataLayer<Dtype> layer(param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(MultiImageDataLayerTest, TestShuffle) {
  ...
  multiimage_data_param->set_batch_size(5);
  multiimage_data_param->set_source(this->filename_.c_str());
  multiimage_data_param->set_shuffle(true);
  MultiImageDataLayer<Dtype> layer(param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_data_->num(), 5);
  EXPECT_EQ(this->blob_top_data_->channels(), 3);
